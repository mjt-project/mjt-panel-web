...
export function bindAuthEvents({ openPanel }) {
  ...
  document.getElementById('tokenInput').addEventListener('keydown', (event) => {
    if (event.key === 'Enter') doLogin(openPanel);
  });
}

export async function doLogin(openPanel) {
  ...
  if (isDevToken(state.token)) {
    state.demo = true;
    saveToken(state.token);
    setLoginMessage('Local dev mode enabled.');
    await openPanel();
    return;
  }
...
async function openDemo(openPanel) {
  state.demo = true;
  state.token = 'demo';
  await openPanel();
}
